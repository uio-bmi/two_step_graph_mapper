...
def get_alle_freq(parts):
    hit = re.search('AF=([0-9.,]+)', parts[7])
    if hit is None:
        logging.error("Found no AF on line %s"  % parts)
    AF = hit.group(1)

    try:
        return max(float(af) for af in AF.split(","))
    except:
        logging.error("Could not parse allele frequency.")
        logging.error(parts)
        logging.error(AF)
        raise
...
def get_common_variants(vcf_filename):
    logging.info("Get common variants")
    lines = (line for line in open(vcf_filename) if not line.startswith("#"))
    parts = (line.split("\t") for line in lines)
    ids = set()
    for j, part in enumerate(parts):
        if j % 100000 == 0:
            logging.info("%d lines parsed " % j)

        if get_alle_freq(part) > 0.01:
...
def get_rare_node_ids(all_vcf, individual_vcf, entry_to_edge):
    logging.info("Get rare node ids")
    ...
    for j, var in enumerate(variants):
        if j % 100000 == 0:
            logging.info("%d variants parsed" % j)

        if type(var) in (vcfmap.INS, vcfmap.SNP):
